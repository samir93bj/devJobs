...
@section('content')
    <h1 class="text-2xl text-center mt-10"><span class="font-bold">CANDIDATOS: </span>{{$vacante->titulo}}</h1>
        ...
        <ul class="text-2xl text-center mt-10">
            @foreach ($vacante->candidatos as $candidato)
                <li class="p-5 border border-gray-400 mb-5">
                    ...
                    <a class="bg-indigo-500 p-3 inline-block text-sm font-bold uppercase text-white" href="/storage/cv/{{$candidato->cv}}">Ver CV</a>
                </li>
            @endforeach
        </ul>
    @else
